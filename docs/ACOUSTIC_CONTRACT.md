...
Finite-flange domain limits apply to each geometry across its whole simulated band. Out-of-domain candidates retain explicit rejection reasons and cannot enter FEM; invalid refinement trials are recorded separately from actual FEM evaluations. If no geometry remains, auto mode emits an explicit no-feasible-design report. Malformed inputs and unrelated solver errors still fail the run.
