...
This dataset can test propagation, termination and repeatability independently of an uncertain loudspeaker motor. It does not validate driver choice or far-field horn output. The existing lossless solver has no finite-flange or thermoviscous wall model, so an unflanged approximation must be labeled as a model-gap diagnostic.
